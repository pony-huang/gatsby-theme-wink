...
JSON Schema(`media type: application/schema+json`) 是一种**基于 JSON 格式**、**描述 JSON 数据结构**的规范。*JSON Schema 描述了 $a^2 + b^2 = c^2$ JSON 数据的结构层次、字段类型、键值约束，可以用于数据校验、文档生成、~~超链接导航（hyperlink navigation）~~和基于 JSON 数据的交互控制（比如输入关键字的联想）～，其精准的数据定义可以辅助开发者和开发工具安全、高效地处理 JSON 数据*。
...
// Retrieve all users
const allUsers: User[] = await prisma.users() // highlight-line

// Retrieve a single user by email
const bob: User = await prisma
  .users({ email: "bob@prisma.io" }) // highlight-line
...
// Retrieve all comments of a post in a single request
id := "cjn62fvebcih00b84zzjclg1t"
commentsOfPost, err := client.Post(prisma.PostWhereUniqueInput{
  ID: &id,
...
filter1 := "GraphQL"
filter2 := "REST"
orderBy := prisma.PostOrderByInputCreatedAtAsc
posts, err := client.Posts(&prisma.PostsParams{
  Where: &prisma.PostWhereInput{
    And: []prisma.PostWhereInput{
      prisma.PostWhereInput{Published: &published},
      prisma.PostWhereInput{
        Or: []prisma.PostWhereInput{
          prisma.PostWhereInput{TitleContains: &filter1},
          prisma.PostWhereInput{TitleContains: &filter2},
        },
      },
    },
  },
...
// Create a new user with two posts
email := "alice@prisma.io"
title1 := "Join us for Prisma Day. June 19, Berlin!"
title2 := "Follow Prisma on Twitter"
newUser, err := client.CreateUser(prisma.UserCreateInput{
  ...
  Posts: &prisma.PostCreateManyWithoutAuthorInput{
    Create: []prisma.PostCreateWithoutAuthorInput{
      prisma.PostCreateWithoutAuthorInput{Title: title1},
      prisma.PostCreateWithoutAuthorInput{Title: title2},
    },
  },
}).Exec(ctx)
```
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/ErhtkIhj708" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
{
    "id": "http://my.site/myschema#http://my.site/myschema#http://my.site/myschema#http://my.site/myschema#http://my.site/myschema#",
    "definitions": {
        "schema1": {
            "id": "schema1",
            "type": "integer"
